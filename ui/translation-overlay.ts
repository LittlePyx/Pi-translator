...
export class TranslationOverlay {
  ...
  private renderResult(result:TranslateResult):void {
    ...
    if(this.alignedView&&result.alignedSegments?.length){
      ...
      for(const [segmentIndex,segment] of result.alignedSegments.entries()){
        ...
        const copy=this.button('复制','mini','复制本句译文');copy.addEventListener('click',()=>void navigator.clipboard.writeText(normalizeLatexForClipboard(segment.translatedText)));
        actions.append(copy);
  ...
  private readonly onKeyDown=(event:KeyboardEvent):void=>{if(this.view==='hidden')return;if(event.key==='Escape'){const correctionCancel=this.root.querySelector<HTMLButtonElement>('.correction-cancel');if(correctionCancel){event.preventDefault();event.stopPropagation();correctionCancel.click();return}if(this.sidebarActive)this.collapseSidebar();else this.closeSurface();return}if(event.altKey&&event.key==='ArrowUp'){event.preventDefault();this.navigate(1)}if(event.altKey&&event.key==='ArrowDown'){event.preventDefault();this.navigate(-1)}};
