...
export class TranslationOverlay {
  ...
  showError(error:ErrorDisplay,rect?:ViewportRect):void {
    ...
    if(showCapturePermissionRecovery){const permission=document.createElement('section');permission.className='capture-permission';permission.setAttribute('aria-label','网页截图授权');const copy=document.createElement('div');copy.className='capture-permission-copy';const title=document.createElement('strong');title.textContent='首次使用需要一次截图授权';const note=document.createElement('span');note.textContent='只截取你主动框选的当前可见区域，授权后自动恢复刚才的框选。';copy.append(title,note);const frame=document.createElement('iframe');frame.className='capture-permission-frame';frame.src=`${this.webCapturePermissionUrl}?theme=${this.host.dataset.piTheme==='dark'?'dark':'light'}`;frame.title='允许 Pi Translator 截取主动框选的网页区域';frame.referrerPolicy='no-referrer';frame.dataset.piFocusTarget='true';permission.append(copy,frame);surface.append(permission)}
    if(showCapturePermissionRecovery){const fallback=this.button('改用浏览器侧栏授权','action capture-permission-fallback');fallback.addEventListener('click',()=>{fallback.disabled=true;fallback.textContent='正在打开浏览器侧栏…';void this.actions.onOpenWebCapturePermissionPanel?.().then(()=>{fallback.textContent='授权入口已打开'}).catch(()=>{fallback.disabled=false;fallback.textContent='重新打开浏览器侧栏'})});footer.append(fallback)}
