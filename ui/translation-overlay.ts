  ...
  private renderHistoryNavigator():void {
    ...
    const renderEntries=()=>{
      ...
      for(const entry of entries){
        ...
        const meta=document.createElement('div');meta.className='history-item-meta';const time=document.createElement('span');time.textContent=new Date(entry.createdAt).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'});meta.append(time);if(entry.sourceHost){const host=document.createElement('span');host.textContent=entry.sourceHost;host.title=entry.sourceHost;meta.append(host)}if(this.actions.hasSourceMarksForResult?.(entry)){const marked=document.createElement('span');marked.className='history-marked';marked.textContent='已标记';meta.append(marked)}if(entry.requestId===this.currentResult?.requestId){const current=document.createElement('span');current.className='history-current';current.textContent='当前';meta.append(current)}
