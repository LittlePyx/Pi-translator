...
export class TranslationOverlay {
  ...
  private showSurface(surface:HTMLDivElement):void {
    const moveFocusIntoCard=this.view==='trigger'&&this.root.activeElement instanceof HTMLElement;
    this.clear();
    ...
    this.setView('card');
    this.scheduleReflow();
    if(moveFocusIntoCard)queueMicrotask(()=>surface.querySelector<HTMLButtonElement>('.header-tools button[aria-label="关闭"]')?.focus());
  }
