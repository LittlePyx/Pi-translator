...
export class TranslationOverlay {
  ...
  destroy():void { if(this.themeTimer)clearTimeout(this.themeTimer);this.themeObserver?.disconnect();this.colorSchemeQuery?.removeEventListener('change',this.onColorSchemeChange);document.removeEventListener('keydown',this.onKeyDown,true);document.removeEventListener('pointerdown',this.onDocumentPointerDown,true);this.host.remove(); }
  ...
  private renderResult(result:TranslateResult):void {
    ...
    if(result.uncertainSpans?.length){const uncertain=document.createElement('div');uncertain.className='uncertain-note';uncertain.textContent=`有 ${result.uncertainSpans.length} 处内容无法完全确认，已在原文中标记。`;surface.append(uncertain)}
  ...
  private moreMenu(result:TranslateResult):HTMLElement {
    ...
    if(result.requestId===this.latestRequestId)menu.append(this.menuButton('重新翻译',this.actions.onRetry));
