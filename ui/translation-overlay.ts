...
import { formatTranslationDuration } from './translation-timing';
...
const STYLES = `
  :host {
    ...
    --line:#dfe5ef; --soft:#f4f7fb; --surface:rgba(255,255,255,.985); --compact-hit:28px;
  ...
  * { box-sizing:border-box; } button,select { font:inherit; } button { cursor:pointer; }
  ...
  .title-wrap { display:flex;align-items:center;gap:8px;min-width:0;margin-right:auto; }.logo{flex:0 0 auto;width:21px;height:18px;object-fit:contain}.title{min-width:0;color:#40506e;font-size:13px;font-weight:780;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.header-tools{display:flex;flex:0 0 auto;align-items:center;gap:2px}
  .live-badge{display:inline-flex;flex:0 0 auto;align-items:center;gap:5px;padding:3px 7px;border-radius:999px;color:#08718b;background:#e7f9fd;font-size:9px;font-weight:750}.live-badge::before{content:"";width:5px;height:5px;border-radius:50%;background:#0ba7c5;box-shadow:0 0 0 3px rgba(11,167,197,.13)}
  ...
  .pin-action{height:var(--compact-hit);padding:0 7px;border:1px solid var(--line);border-radius:5px;color:#4b5870;background:transparent;font-size:10px;font-weight:680;white-space:nowrap}.pin-action:hover{color:var(--accent);border-color:#b8c0ea;background:var(--soft)}
  .result-topline{display:flex;align-items:center;gap:6px;min-height:26px;margin-top:5px}.meta{display:flex;flex:1;flex-wrap:wrap;align-items:center;gap:5px;min-width:0;color:var(--muted);font-size:9px}.meta-dot::before{content:"·";margin-right:5px}.cache-badge{padding:0;color:#16839a;background:transparent;font-weight:650}.source-location{min-height:24px;padding:0 2px;border:0;border-bottom:1px solid transparent;color:var(--muted);background:transparent;font-size:9px}.source-location:hover{color:var(--accent);border-bottom-color:currentColor}
  ...
  .segment-actions{display:flex;gap:5px;margin-top:7px;opacity:0;transition:.15s opacity}.segment:hover .segment-actions,.segment:focus-within .segment-actions{opacity:1}.mini{min-height:var(--compact-hit);padding:0 8px;border:0;border-radius:6px;color:var(--muted);background:var(--soft);font-size:10px}.mini:hover{color:var(--accent)}
  .segment-correction{display:grid;gap:5px;padding-top:6px;border-top:1px dashed var(--line)}.segment-correction .correction-text-part{min-height:56px;padding:5px 1px}.segment-correction-actions{display:flex;align-items:center;justify-content:flex-end;gap:4px}.segment-correction-status{margin-right:auto;color:var(--muted);font-size:9px}
  ...
  :host([data-pi-theme="dark"]) .stop-translation:hover{color:#ff9aa4;background:var(--soft)}:host([data-pi-theme="dark"]) .pi-math-display::-webkit-scrollbar-thumb,:host([data-pi-theme="dark"]) .pi-math-scroll::-webkit-scrollbar-thumb{background:rgba(169,181,199,.48)}
  @media(max-width:420px){.correction-term-fields,.document-edit{grid-template-columns:minmax(0,1fr)}.document-edit-actions{justify-content:flex-end}.revision-actions{position:sticky;bottom:0;padding:6px 0;background:var(--surface)}}
  ...
  @media(max-width:620px){:host{--compact-hit:32px}.sidebar{top:calc(var(--pi-viewport-top) + 8px)!important;right:calc(var(--pi-viewport-right) + 8px)!important;bottom:calc(var(--pi-viewport-bottom) + 8px)!important;left:calc(var(--pi-viewport-left) + 8px)!important;width:auto!important}.sidebar-resizer{display:none}.icon{width:var(--compact-hit);height:var(--compact-hit)}.stop-translation{padding:0 8px;border:1px solid var(--line);border-radius:6px;font-size:10.5px}.segment-actions{opacity:1}}
...
export class TranslationOverlay {
  ...
  private renderResult(result:TranslateResult):void {
    ...
    if(this.alignedView&&result.alignedSegments?.length){
      ...
      for(const [segmentIndex,segment] of result.alignedSegments.entries()){
        const row=document.createElement('section');row.className='segment';row.tabIndex=0;row.dataset.segmentId=segment.id;
  ...
  private openSegmentCorrection(
    ...
    const controls=document.createElement('div');controls.className='segment-correction-actions';const status=document.createElement('span');status.className='segment-correction-status';status.setAttribute('role','status');status.setAttribute('aria-live','polite');const cancel=this.button('取消','mini correction-cancel');const save=this.button('保存','mini');controls.append(status,cancel,save);editor.append(controls);pair.hidden=true;segmentActions.hidden=true;content.append(editor);
    const restoreFocus=()=>{this.renderResult(result);queueMicrotask(()=>{const row=[...this.root.querySelectorAll<HTMLElement>('.segment')].find(candidate=>candidate.dataset.segmentId===segment.id);(row?.querySelector<HTMLButtonElement>('.segment-correct')??row??trigger).focus()})};
    ...
    save.addEventListener('click',()=>{const edits:ManualCorrectionEdit[]=[...inputs].map(([partId,input])=>({partId,text:input.value}));let corrected:string;try{corrected=applyManualCorrection(session,{revision:draft.revision,edits}).correction.correctedTranslation}catch(error){status.textContent=error instanceof ManualCorrectionError?(error.code==='NO_CHANGES'?'本句译文没有变化':error.code==='LATEX_CHANGED'?'公式已锁定，请只修改文字':'修正内容不完整，请检查'):error instanceof Error?error.message:'无法保存本句修正';return}save.disabled=true;cancel.disabled=true;for(const input of inputs.values())input.disabled=true;editor.setAttribute('aria-busy','true');status.textContent='正在保存…';void this.actions.onSaveSegmentTranslationEdit!(result,segment.id,segment.translatedText,corrected).then((response)=>{if(this.translationEpoch!==editEpoch||this.latestRequestId!==editRequestId)return;this.correctionUndo=response.correctionReceipt;this.showResult(response.result,undefined,response.history,true);queueMicrotask(()=>{const row=[...this.root.querySelectorAll<HTMLElement>('.segment')].find(candidate=>candidate.dataset.segmentId===segment.id);(row?.querySelector<HTMLButtonElement>('.segment-correct')??row)?.focus()})}).catch((error:unknown)=>{save.disabled=false;cancel.disabled=false;for(const input of inputs.values())input.disabled=false;editor.removeAttribute('aria-busy');status.textContent=error instanceof Error?error.message:'保存失败，请重试'})});
    queueMicrotask(()=>{(inputs.values().next().value??cancel).focus()});
  }

  private openTranslationCorrection(result:TranslateResult,trigger:HTMLButtonElement):void {
    ...
    const note=document.createElement('p');note.className='revision-note';note.textContent=formulaNumber?'公式片段已锁定；保存只更新自然语言，不调用 API。':'保存修改不会调用 API。';if(result.alignedSegments?.length)note.textContent+=' 保存整段修正后将退出逐句对照。';panel.append(note);
    ...
    save.addEventListener('click',()=>{const selectedScope=scope.value as TranslationMemoryScope;const edits:ManualCorrectionEdit[]=[...inputs].map(([partId,input])=>({partId,text:input.value}));const hasTermInput=Boolean(source.value.trim()||target.value.trim());const explicitTermCandidate=termDisclosure.open&&hasTermInput?{source:source.value,target:target.value}:undefined;let corrected:string;let term:TranslationCorrectionTermInput|undefined;try{const applied=applyManualCorrection(session,{revision:draft.revision,edits,...(explicitTermCandidate?{explicitTermCandidate}:{})});corrected=applied.correction.correctedTranslation;term=applied.correction.termCandidateDraft?{source:applied.correction.termCandidateDraft.source,target:applied.correction.termCandidateDraft.target,scope:termScope.value as TranslationCorrectionTermInput['scope']}:undefined}catch(error){status.textContent=error instanceof ManualCorrectionError?(error.code==='NO_CHANGES'?'译文没有变化':error.code==='LATEX_CHANGED'?'公式已锁定，请只修改文字':error.code==='INVALID_TERM_CANDIDATE'?'请完整填写不含公式的简短术语和固定译法':'修正内容不完整，请检查'):error instanceof Error?error.message:'无法保存修正';return}save.disabled=true;cancel.disabled=true;scope.disabled=true;termScope.disabled=true;for(const input of inputs.values())input.disabled=true;source.disabled=true;target.disabled=true;panel.setAttribute('aria-busy','true');status.textContent='正在保存…';void this.actions.onSaveTranslationEdit!(result,corrected,selectedScope,term).then((response)=>{if(this.translationEpoch!==editEpoch||this.latestRequestId!==editRequestId)return;this.correctionUndo=response.correctionReceipt;this.showResult(response.result,undefined,response.history,false);queueMicrotask(()=>this.root.querySelector<HTMLButtonElement>('.correction-undo button')?.focus())}).catch((error:unknown)=>{save.disabled=false;cancel.disabled=false;scope.disabled=false;termScope.disabled=false;for(const input of inputs.values())input.disabled=false;source.disabled=false;target.disabled=false;panel.removeAttribute('aria-busy');status.textContent=error instanceof Error?error.message:'保存失败，请重试'})});
  ...
  private undoTranslationCorrection(result:TranslateResult,notice:HTMLElement):void {
    const receipt=this.correctionUndo;if(!receipt||!this.actions.onUndoTranslationEdit)return;const epoch=this.translationEpoch;notice.setAttribute('aria-busy','true');const button=notice.querySelector<HTMLButtonElement>('button');if(button)button.disabled=true;void this.actions.onUndoTranslationEdit(result,receipt).then((response)=>{if(epoch!==this.translationEpoch||this.currentResult?.requestId!==result.requestId)return;this.correctionUndo=undefined;this.showResult(response.result,undefined,response.history,Boolean(receipt.segmentChange));if(response.termRollbackSkipped){const status=document.createElement('span');status.className='correction-undo';status.setAttribute('role','status');status.textContent='译文已撤销；术语后来被修改，未自动覆盖';this.root.querySelector('.footer')?.prepend(status)}queueMicrotask(()=>{const segmentId=receipt.segmentChange?.segmentId;if(segmentId){const row=[...this.root.querySelectorAll<HTMLElement>('.segment')].find(candidate=>candidate.dataset.segmentId===segmentId);if(row){(row.querySelector<HTMLButtonElement>('.segment-correct')??row).focus();return}}this.root.querySelector<HTMLButtonElement>('.correction-action')?.focus()})}).catch((error:unknown)=>{notice.removeAttribute('aria-busy');if(button)button.disabled=false;notice.firstChild!.textContent=error instanceof Error?`${error.message} ·`:'撤销失败 ·'})
  }

  private openModelAdjustment(result:TranslateResult):void {
    if(!this.actions.onAdjustTranslation)return;
    const surface=this.surface('模型调整');const panel=document.createElement('div');panel.className='revision-panel';
    ...
    const actions=document.createElement('div');actions.className='revision-actions';const cancel=this.button('返回','action');cancel.addEventListener('click',()=>this.renderResult(result));actions.append(cancel);panel.append(actions);
  ...
  private recognizedSource(result:TranslateResult,label:string):HTMLElement {
    ...
    edit.addEventListener('click',()=>{const editor=document.createElement('textarea');editor.className='recognized-editor';editor.value=recognizedText;editor.setAttribute('aria-label','编辑识别原文');source.replaceWith(editor);actions.replaceChildren();const commit=this.button('用修正文本重译','commit');const cancel=this.button('取消编辑','correction-cancel');actions.append(commit,cancel);commit.addEventListener('click',()=>{const text=editor.value.trim();if(!text){editor.focus();return}this.actions.onTranslateText(text)});cancel.addEventListener('click',()=>{this.renderResult(result);queueMicrotask(()=>this.root.querySelector<HTMLButtonElement>('.recognized-source summary')?.focus())});queueMicrotask(()=>{editor.focus();editor.setSelectionRange(editor.value.length,editor.value.length)})});
    return recognized;
  }
