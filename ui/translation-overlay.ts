...
export class TranslationOverlay {
  ...
  private translatedTextElement(text:string,className:string,renderLatex:boolean):HTMLDivElement {
    ...
    for(const segment of splitLatexDisplaySegments(text)){
      ...
      const math=document.createElement(segment.displayMode?'div':'span');math.className=`pi-math ${segment.displayMode?'pi-math-display':'pi-math-inline'}`;math.textContent=segment.raw;element.append(math);void browser.runtime.sendMessage({type:'RENDER_LATEX_MATHML',payload:{tex:segment.tex,displayMode:segment.displayMode}} satisfies RuntimeMessage).then((response:LatexMathMlResponse)=>{if(response.ok&&response.data.html&&math.isConnected)math.innerHTML=response.data.html}).catch(()=>undefined)
    }
    return element;
  }
