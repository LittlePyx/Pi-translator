...
import type { LatexMathMlResponse, RuntimeMessage } from '../core/messaging/messages';
import type {
...
const STYLES = `
  ...
  * { box-sizing:border-box; } button,select { font:inherit; } button { cursor:pointer; }
  ...
  .body { margin-top:8px;font-size:14px;line-height:1.78;white-space:pre-wrap;overflow-wrap:anywhere; }.pi-math-inline{display:inline-flex;max-width:100%;vertical-align:-.14em;white-space:normal}.pi-math-display{display:block;width:100%;margin:.55em 0;overflow-x:auto;text-align:center;white-space:normal}.pi-math-display.pi-math-numbered{display:grid;grid-template-columns:minmax(0,1fr) auto;align-items:center;gap:8px;overflow:visible}.pi-math-scroll{min-width:0;overflow-x:auto;text-align:center;scrollbar-width:thin}.pi-equation-tag{align-self:center;white-space:nowrap;font-family:"Cambria Math","STIX Two Math",serif}.pi-math .katex{color:inherit;font-size:1.02em}.pi-math math{color:inherit}.progress{margin-top:10px}.loading{display:flex;align-items:center;gap:10px;padding:10px 0;color:var(--muted);font-size:12px}.spinner{flex:0 0 auto;width:17px;height:17px;border:2px solid #cdd5e5;border-top-color:var(--accent);border-radius:50%;animation:spin .8s linear infinite}.stream-preview{max-height:300px;margin-top:5px;padding:11px;border-radius:12px;background:var(--soft);font-size:13px;line-height:1.72;white-space:pre-wrap;overflow:auto}.stream-preview[hidden]{display:none}@keyframes spin{to{transform:rotate(360deg)}}
...
export class TranslationOverlay {
  ...
  private renderDocumentMemory():void {
    ...
    const recent=document.createElement('section');recent.className='document-section';const recentHead=document.createElement('div');recentHead.className='document-section-head';const recentTitle=document.createElement('strong');recentTitle.textContent='最近翻译';const recentCount=document.createElement('span');recentCount.textContent=String(memory.recentTranslations.length);recentHead.append(recentTitle,recentCount);recent.append(recentHead);const recentList=document.createElement('div');recentList.className='document-list';for(const entry of memory.recentTranslations){const button=this.button('','document-translation','查看这条翻译');const source=document.createElement('div');source.className='document-source';source.textContent=entry.originalText;const target=document.createElement('div');target.className='document-target';target.textContent=entry.translatedText;button.append(source,target);button.addEventListener('click',()=>this.openDocumentTranslation(entry));recentList.append(button)}if(!memory.recentTranslations.length){const empty=document.createElement('div');empty.className='document-empty';empty.textContent='本文完成的翻译会出现在这里。';recentList.append(empty)}recent.append(recentList);surface.append(recent);
  ...
  private translatedTextElement(text:string,className:string,renderLatex:boolean):HTMLDivElement {
    ...
    if(!renderLatex){element.textContent=text;return element}
    for(const segment of splitLatexDisplaySegments(text)){
      if(segment.kind==='text'){element.append(document.createTextNode(segment.text));continue}
      ...
      math.textContent=segment.raw;element.append(math);
      const renderParts=latexRenderParts(segment.tex,segment.displayMode);
      void browser.runtime.sendMessage({type:'RENDER_LATEX_MATHML',payload:{tex:renderParts.tex,displayMode:segment.displayMode}} satisfies RuntimeMessage).then((response:LatexMathMlResponse)=>{
        if(!response.ok||!response.data.html||!math.isConnected)return;
        if(!renderParts.equationTag){math.innerHTML=response.data.html;return}
        const scroll=document.createElement('span');scroll.className='pi-math-scroll';scroll.innerHTML=response.data.html;
        const tag=document.createElement('span');tag.className='pi-equation-tag';tag.textContent=`(${renderParts.equationTag})`;
        math.classList.add('pi-math-numbered');math.replaceChildren(scroll,tag)
      }).catch(()=>undefined)
    }
    return element;
  }
  ...
  private renderResult(result:TranslateResult):void {
    ...
    const renderLatex=this.shouldRenderLatex(result);if(this.alignedView&&result.alignedSegments?.length){const list=document.createElement('div');list.className='aligned-list';for(const segment of result.alignedSegments){const row=document.createElement('section');row.className='segment';row.tabIndex=0;const num=document.createElement('span');num.className='segment-number';num.textContent=segment.id.replace(/^S/,'');const content=document.createElement('div');const pair=document.createElement('div');pair.className='segment-pair';const source=document.createElement('div');source.className='segment-source';source.textContent=segment.originalText;const target=this.translatedTextElement(segment.translatedText,'segment-target',renderLatex);pair.append(source,target);const actions=document.createElement('div');actions.className='segment-actions';const copy=this.button('复制本句','mini');copy.addEventListener('click',()=>void navigator.clipboard.writeText(segment.translatedText));const retry=this.button('仅翻译此句','mini');retry.addEventListener('click',()=>this.actions.onTranslateText(segment.originalText));actions.append(copy,retry);if(this.actions.canMarkSource?.(result,segment)){const mark=this.button('','mini segment-mark',this.actions.isSourceMarked?.(result,segment)?'取消本句标记':'轻标记本句');mark.append(this.markerIcon());mark.classList.toggle('active',Boolean(this.actions.isSourceMarked?.(result,segment)));mark.setAttribute('aria-pressed',String(Boolean(this.actions.isSourceMarked?.(result,segment))));mark.addEventListener('click',()=>this.toggleSegmentSourceMark(result,segment));actions.append(mark)}content.append(pair,actions);row.append(num,content);list.append(row)}surface.append(list)}else{surface.append(this.translatedTextElement(result.translatedText,'body',renderLatex))}
