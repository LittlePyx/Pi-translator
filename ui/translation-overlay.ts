...
const STYLES = `
  ...
  * { box-sizing:border-box; } button,select { font:inherit; } button { cursor:pointer; }
  ...
  :host([data-pi-theme="dark"]) .document-memory-action.has-review,:host([data-pi-theme="dark"]) .document-review-meta strong{color:#f1d68e}:host([data-pi-theme="dark"]) .document-review-actions .review-resolve{color:var(--muted)}:host([data-pi-theme="dark"]) .correction-undo.is-error{color:#ff9aa4}
  ...
  @media(max-width:620px){:host{--compact-hit:32px}.sidebar{top:calc(var(--pi-viewport-top) + 8px)!important;right:calc(var(--pi-viewport-right) + 8px)!important;bottom:calc(var(--pi-viewport-bottom) + 8px)!important;left:calc(var(--pi-viewport-left) + 8px)!important;width:auto!important}.sidebar-resizer{display:none}.icon{width:var(--compact-hit);height:var(--compact-hit)}.view-button{height:var(--compact-hit)}.source-location{min-height:var(--compact-hit)}.stop-translation{padding:0 8px;border:1px solid var(--line);border-radius:6px;font-size:10.5px}.recognized-source summary{display:flex;align-items:center;min-height:var(--compact-hit)}.recognized-source summary::after{float:none;margin-left:auto}.segment-actions{opacity:1}}
...
export class TranslationOverlay {
  ...
  private openSegmentCorrection(
    ...
    save.addEventListener('click',()=>{const edits:ManualCorrectionEdit[]=[...inputs].map(([partId,input])=>({partId,text:input.value}));let corrected:string;try{corrected=applyManualCorrection(session,{revision:draft.revision,edits}).correction.correctedTranslation}catch(error){status.textContent=error instanceof ManualCorrectionError?(error.code==='NO_CHANGES'?'本句译文没有变化':error.code==='LATEX_CHANGED'?'公式已锁定，请只修改文字':'修正内容不完整，请检查'):error instanceof Error?error.message:'无法保存本句修正';return}const viewportOffset=this.segmentViewportOffset(segment.id);save.disabled=true;cancel.disabled=true;for(const input of inputs.values())input.disabled=true;editor.setAttribute('aria-busy','true');status.textContent='正在保存…';void this.actions.onSaveSegmentTranslationEdit!(result,segment.id,segment.translatedText,corrected).then((response)=>{if(this.translationEpoch!==editEpoch||this.latestRequestId!==editRequestId)return;this.correctionUndo=response.correctionReceipt;this.showResult(response.result,undefined,response.history,true);this.restoreSegmentReadingPosition(response.result,segment.id,viewportOffset)}).catch((error:unknown)=>{save.disabled=false;cancel.disabled=false;for(const input of inputs.values())input.disabled=false;editor.removeAttribute('aria-busy');status.textContent=error instanceof Error?error.message:'保存失败，请重试'})});
  ...
  private segmentViewportOffset(segmentId:string):number|undefined { const surface=this.root.querySelector<HTMLElement>('.surface');const row=this.segmentRow(segmentId);if(!surface||!row)return undefined;return row.getBoundingClientRect().top-surface.getBoundingClientRect().top }
  private restoreSegmentReadingPosition(result:TranslateResult,segmentId:string,viewportOffset?:number):void {
    ...
    const restore=()=>{
      ...
      const surface=this.root.querySelector<HTMLElement>('.surface');const row=this.segmentRow(segmentId);if(!surface||!row)return;
      if(viewportOffset!==undefined)surface.scrollTop+=row.getBoundingClientRect().top-surface.getBoundingClientRect().top-viewportOffset;
      const target=row.querySelector<HTMLElement>('.segment-target')??row;const surfaceBounds=surface.getBoundingClientRect();const headerBottom=surface.querySelector<HTMLElement>(':scope > .header')?.getBoundingClientRect().bottom??surfaceBounds.top;const visibleTop=Math.max(surfaceBounds.top,headerBottom)+8;const visibleBottom=surfaceBounds.bottom-8;const targetBounds=target.getBoundingClientRect();
      if(targetBounds.height>visibleBottom-visibleTop||targetBounds.top<visibleTop)surface.scrollTop+=targetBounds.top-visibleTop;
      else if(targetBounds.bottom>visibleBottom)surface.scrollTop+=targetBounds.bottom-visibleBottom;
    };
  ...
  private renderResultPreservingScroll(result:TranslateResult,scrollTop=this.root.querySelector<HTMLElement>('.surface')?.scrollTop??0):void { const requestId=result.requestId;this.renderResult(result);const restore=()=>{if(this.currentResult?.requestId!==requestId)return;const surface=this.root.querySelector<HTMLElement>('.surface');if(surface)surface.scrollTop=scrollTop};restore();queueMicrotask(restore);requestAnimationFrame(restore) }
  private renderAfterMarkToggle(result:TranslateResult,preserveScroll=false):void { const scrollTop=preserveScroll?this.root.querySelector<HTMLElement>('.surface')?.scrollTop??0:0;if(this.markedOnly&&!this.actions.hasSourceMarksForResult?.(result)){const next=this.history.find(entry=>this.actions.hasSourceMarksForResult?.(entry));if(next){this.alignedView=false;this.renderResult(next);return}this.markedOnly=false}if(preserveScroll)this.renderResultPreservingScroll(result,scrollTop);else this.renderResult(result) }
