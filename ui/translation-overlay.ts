...
const STYLES = `
  ...
  * { box-sizing:border-box; } button,select { font:inherit; } button { cursor:pointer; }
  ...
  .trigger-placement{position:fixed;z-index:2147483647;display:flex;align-items:stretch;gap:6px;max-width:min(360px,calc(100vw - var(--pi-viewport-left) - var(--pi-viewport-right) - 24px));pointer-events:none}.trigger-placement .trigger{position:relative;flex:0 0 38px}.selection-preview{display:grid;align-content:center;min-width:0;width:min(288px,calc(100vw - var(--pi-viewport-left) - var(--pi-viewport-right) - 68px));padding:6px 9px;border:1px solid rgba(99,102,241,.16);border-radius:8px;color:var(--muted);background:var(--surface);box-shadow:0 9px 28px rgba(30,41,59,.17);font-size:9.5px;line-height:1.42}.selection-preview-text{overflow:hidden;color:var(--text);text-overflow:ellipsis;white-space:nowrap}.selection-preview-warning{overflow:hidden;margin-top:2px;color:#936315;text-overflow:ellipsis;white-space:nowrap}:host([data-pi-theme="dark"]) .selection-preview-warning{color:#efd68f}
...
export class TranslationOverlay {
  ...
  showTrigger(rect:ViewportRect,preview?:SelectionTriggerPreview):void {
    ...
    this.refreshViewportInsets();if(preview?.text){const placement=document.createElement('div');placement.className='trigger-placement';const summary=document.createElement('div');summary.className='selection-preview';summary.setAttribute('aria-label','发送前选区预览');const text=document.createElement('div');text.className='selection-preview-text';text.textContent=preview.text;text.title=preview.text;summary.append(text);if(preview.warning){const warning=document.createElement('div');warning.className='selection-preview-warning';warning.textContent=preview.warning;warning.title=preview.warning;summary.append(warning)}placement.append(summary,button);this.root.append(placement);this.place(placement,rect);this.observeSize(placement)}else{this.root.append(button);this.place(button,rect);this.observeSize(button)}this.setView('trigger');
  }
  ...
  private reflowVisibleSurface():void {
    ...
    if(this.view==='trigger'){
      const trigger=this.root.querySelector<HTMLElement>('.trigger-placement,.trigger');
