...
export class TranslationOverlay {
  ...
  private openTranslationCorrection(result:TranslateResult,trigger:HTMLButtonElement):void {
    ...
    const scopeLabel=document.createElement('label');scopeLabel.className='revision-scope';const scopeText=document.createElement('span');scopeText.textContent='保存方式';const scope=document.createElement('select');scope.ariaLabel='修正后的使用范围';for(const [value,label] of [['current','仅修正本次'],['document','用于本文并固定术语'],['global','同时固定为全局术语']] as const){const option=document.createElement('option');option.value=value;option.textContent=label;scope.append(option)}scopeLabel.append(scopeText,scope);panel.append(scopeLabel);
    const termFields=document.createElement('fieldset');termFields.className='correction-term-fields';termFields.hidden=true;const sourceLabel=document.createElement('label');sourceLabel.textContent='原文术语';const source=document.createElement('input');source.maxLength=120;source.placeholder='例如 adaptive sensing';source.ariaLabel='原文术语';sourceLabel.append(source);const targetLabel=document.createElement('label');targetLabel.textContent='固定译法';const target=document.createElement('input');target.maxLength=120;target.placeholder='例如 自适应感知';target.ariaLabel='固定译法';targetLabel.append(target);termFields.append(sourceLabel,targetLabel);panel.append(termFields);
    scope.addEventListener('change',()=>{termFields.hidden=scope.value==='current'});
    ...
    save.addEventListener('click',()=>{const selectedScope=scope.value as TranslationRevisionScope;const edits:ManualCorrectionEdit[]=[...inputs].map(([partId,input])=>({partId,text:input.value}));const explicitTermCandidate=selectedScope==='current'?undefined:{source:source.value,target:target.value};let corrected:string;let term:GlossaryEntry|undefined;try{const applied=applyManualCorrection(session,{revision:draft.revision,edits,...(explicitTermCandidate?{explicitTermCandidate}:{})});corrected=applied.correction.correctedTranslation;term=applied.correction.termCandidateDraft?{source:applied.correction.termCandidateDraft.source,target:applied.correction.termCandidateDraft.target}:undefined}catch(error){status.textContent=error instanceof ManualCorrectionError?(error.code==='NO_CHANGES'?'译文没有变化':error.code==='LATEX_CHANGED'?'公式已锁定，请只修改文字':error.code==='INVALID_TERM_CANDIDATE'?'请填写不含公式的简短术语和固定译法':'修正内容不完整，请检查'):error instanceof Error?error.message:'无法保存修正';return}save.disabled=true;cancel.disabled=true;scope.disabled=true;for(const input of inputs.values())input.disabled=true;source.disabled=true;target.disabled=true;panel.setAttribute('aria-busy','true');status.textContent='正在保存…';void this.actions.onSaveTranslationEdit!(result,corrected,selectedScope,term).then((response)=>{if(this.translationEpoch!==editEpoch||this.latestRequestId!==editRequestId)return;this.correctionUndo=response.correctionReceipt;this.showResult(response.result,undefined,response.history,false);queueMicrotask(()=>this.root.querySelector<HTMLButtonElement>('.correction-undo button')?.focus())}).catch((error:unknown)=>{save.disabled=false;cancel.disabled=false;scope.disabled=false;for(const input of inputs.values())input.disabled=false;source.disabled=false;target.disabled=false;panel.removeAttribute('aria-busy');status.textContent=error instanceof Error?error.message:'保存失败，请重试'})});
