...
visionApiProfile.addEventListener('change',()=>{visionProfileId=visionApiProfile.value;setDirty()});
...
visionApiProfile.addEventListener('change',()=>setVisionTestStatus('发送一张内置的 16×16 测试图，不含文档内容。'));
...
testVisionCapabilityButton.addEventListener('click',()=>{void(async()=>{
  ...
  const response=await browser.runtime.sendMessage({type:'TEST_VISION_CAPABILITY',payload:{apiBaseUrl:profile.apiBaseUrl,model,profileId:profile.id,...(pendingKey?{apiKey:pendingKey}:{})}} satisfies RuntimeMessage) as VisionCapabilityTestResponse;
