...
function renderProfileSelect():void {
  ...
  profileSelect.value=currentProfileId;deleteProfileButton.disabled=profiles.length<=1;
}
...
async function refreshKeyState():Promise<void>{apiKeyState.textContent=(await hasApiKey(currentProfileId))?'当前配置已保存 Key；留空可保留':'当前配置尚未保存 Key'}
