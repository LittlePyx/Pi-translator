...
| 分支 | `main`；发布准备提交后相对 `origin/main` ahead 21 |
| 功能基线 | `55e247a docs: prepare v0.14.0 release candidate`；其后只有交接及 v0.15.0 发布元数据 |
| 当前版本 | `0.15.0` |
| 发布候选 | `.output/tex-selection-translator-0.15.0-edge.zip` |
| ZIP 大小 | 1,149,285 bytes |
| SHA-256 | `E8BD0512DD3DFD2235F86BE2072BD23E0CA0FE3FD1366CAFBAA2AFB8889C5A6B` |
| ZIP 内容 | 33 项，Manifest V3；与生产构建逐文件一致 |
| 发布状态 | 尚未推送本地增量，未创建 `v0.15.0` tag、GitHub Release 或 Edge 商店提交 |
...
本轮只调整了版本、发布说明、验收记录和三个开发工具链传递依赖，没有修改运行时功能逻辑。
...
最终 ZIP 还从解压目录直接加载到真实 Edge：Manifest V3、版本 `0.15.0`，设置页、快捷面板和 Pi PDF 均正常，页面错误为 0。它与此前完成完整实机验收的 ZIP 比较，只有 `manifest.json` 的版本号不同，其余 32 个文件逐字节一致；完整实机验收已覆盖：
...
1. 检查发布准备提交只包含 package、lock、CHANGELOG 和 v0.15.0 发布文档，不包含 `.output/`、广告素材或凭证。
2. 等待用户明确授权后推送 `main`。
3. 确认当前提交的 GitHub Actions 全部绿色；旧 `0d0428c` 的公开 CI 失败不能替代当前候选的云端验证。
4. 云端绿灯后再次取得用户授权，再创建签名 `v0.15.0` tag、GitHub Release 并上传上述 ZIP。
5. 最后按审核材料提交 Edge 商店；真实审核 Key 只能填写到 Partner Center，不得保存进仓库。
