...
- Pi PDF 支持框选局部图片或扫描文字，通过单独配置的视觉模型完成识别与翻译；确认前不会发送图片，选框可移动、四角可缩放，识别原文默认折叠显示。
- 保留 Edge 原生 PDF 阅读器：选中文字后右键翻译，结果会在 Edge 侧边栏流式显示；侧边栏仅提示可按需用 Pi PDF 打开，不会自动跳转或替换原生阅读器。
- PDF 页面按视口附近懒加载，长论文不会在打开时一次性渲染全部页面；扫描版缺少文字层时只显示一次轻量提示。
...
4. 有文字层时直接使用原生划词；扫描版或模糊 PDF 只需点一次小型“框选翻译”入口，再拖出选区。

本地 PDF 的完整文件只在当前扩展标签页中解析，不会上传给 P&I Lab 或翻译 API。文字划选时只发送用户主动触发的文字；图像区域翻译时，只在用户拖框并点击“翻译”确认后发送该局部截图。截图最长边会缩放到 2048 像素以内、大小限制约 3 MB，不写入历史、缓存、收藏、设置或诊断记录。
